...
export default function TransferCard({ updateBalance }) {
  ...
  async function handleClick() {
    ...
    const res = await fetch(`${BASE_URL}/api/account`, {
      ...
      body: JSON.stringify({ action: "transferFunds", amount: parseInt(amount), session: session }),
    });
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>
          <ArrowUpRight size={20} className="text-primary" /> Transfer
        </CardTitle>
